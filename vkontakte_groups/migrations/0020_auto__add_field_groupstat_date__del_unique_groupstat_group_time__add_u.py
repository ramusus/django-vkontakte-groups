...
from vkontakte_groups.models import GroupStat
...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        # save date field
        for group_stat in GroupStat.objects.all():
            group_stat.date = group_stat.time.date()
            group_stat.save()

        # remove duplicates
        for group_stat in GroupStat.objects.order_by('group','-time'):
            try:
                GroupStat.objects.get(id=group_stat.id)
            except:
                continue
            duplicates = GroupStat.objects.filter(group=group_stat.group, date=group_stat.date).exclude(id=group_stat.id)
            if len(duplicates):
                duplicates.delete()
