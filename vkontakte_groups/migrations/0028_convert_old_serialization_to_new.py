...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        for i in range(0,GroupStatMembers.objects.count()):
            for gm in GroupStatMembers.objects.order_by('id')[i:i+1]:
                for field_name in ['members','members_entered','members_left','members_deactivated_entered','members_deactivated_left','members_has_avatar_entered','members_has_avatar_left']:
                    print gm.id, gm.time, field_name
                    value = getattr(gm, field_name)
                    ids = [int(v) for v in value.split(',') if v] if value else []
                    setattr(gm, field_name + '_ids', ids)
                gm.save()
